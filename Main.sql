...
--No commits should be made until final SELECT statement executed.
--Ensure the materialized views are refreshed before PRODUCTION
--Ensure that the hard-coded grouping in the main SELECT is modified based on your organization details
--We have one specific inventory organization that raises POs, however receives the materials in a different organization
--that is WMS enabled. Grouping the data is mandatory for us for reporting

SET SERVEROUTPUT ON SIZE UNLIMITED
/

--
--CASE #1 When you have the privileges to create custom objects
--
Declare
p_org_id NUMBER :=110;
l_warehouse_orgid NUMBER:= 245;
BEGIN
		BEGIN
				INSERT INTO XXAGEING_RCPTS_GTT (organization_id, transaction_date, inventory_item_id, transaction_quantity)
				SELECT 
					mmt.organization_id,
					mmt.receipt_date, 
					mmt.inventory_item_id,
					mmt.transaction_quantity
				FROM OMS_AGING_RECEIPTS_MV mmt
				WHERE 
				1=1
				...
				EXCEPTION
				   WHEN OTHERS THEN
					  -- Capture and print the exact error message
					  DBMS_OUTPUT.PUT_LINE('Insert failed! Error Code: ' || SQLCODE);
					  DBMS_OUTPUT.PUT_LINE('Error Message: ' || SQLERRM);
					  ROLLBACK; -- Undo changes if part of a multi-step transaction
		END;
			  
		BEGIN	  

				INSERT INTO XXAGEING_OHQTY_GTT (
					  --organization_id, --not mandatory for current requirement
					  inventory_item_id, current_on_hand)
					  SELECT 
					  --moq.organization_id, --not mandatory for current requirement
					  moq.inventory_item_id, SUM(moq.transaction_quantity)
...
Select mmt.organization_id, mmt.transaction_date, mmt.inventory_item_id, mmt.transaction_quantity 
    from XXAGEING_RCPTS_GTT mmt
  ),
on_hand AS (
    -- Input parameters: item_id and current stock on hand
    select moq.inventory_item_id, moq.current_on_hand
    from XXAGEING_OHQTY_GTT moq
 ),
running_totals AS (
    SELECT 
...
/	
commit --flush the global temporary tables used in the current session
/
